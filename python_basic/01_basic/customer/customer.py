    ...
    if choice=="I":
        ...
        while True:
            ...
            if m:
                ...
                print('@가 포함되어 있지 않습니다.')
        while True:
            birthyear = input('출생년도를 입력하세요(4자리) >>> ')
            birthleng = len(birthyear)
            if birthleng == 4:
                break
            else:
                print('4자리가 아닙니다.')

        cus_dic = {'name' : name, 'gender' : gender, 'email' : email, 'birthyear' : birthyear}

        custlist.append(cus_dic)
        ...
        if page < 0:
            ...
            print(f'현재 입력된 페이지는 {page + 1}페이지 입니다.')
        print(custlist)
    elif choice == 'P':
        if page <= 0 :
            print('첫번째 페이지입니다. 이전 페이지 이동 불가합니다.')
        else:
            page -= 1
